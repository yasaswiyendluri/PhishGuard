...
function ReportsTable({ scans = [], loading }) {
    if (loading) {
        return (
            <div className="panel p-8 mt-8 text-center text-slate-500">Loading scan history…</div>
        );
    }
                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                        {scans.map((scan) => (
                            ...
                            >
                                <td
                                    className="py-4 pr-4 max-w-xs truncate mono text-slate-300"
                                    title={scan.url}
                                >
                                    {scan.url}
                                </td>
                                ...
                                <td className="py-4 pr-4 text-cyan-300 font-medium">
                                    {mlDisplay(scan)}
                                </td>
